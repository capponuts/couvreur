  ...
  return (
    <main className="min-h-screen">
      ...
      <section className="relative h-[44vh] md:h-[54vh]">
        <div className="absolute inset-0" style={{ backgroundImage: "url('/real4.jpg')", backgroundSize: 'cover', backgroundPosition: 'center' }} />
        <div className="absolute inset-0 bg-gradient-to-t from-white/30 via-white/10 to-transparent" />
        <div className="relative z-10 h-full flex items-center justify-center text-center px-6">
          <div>
            <h1 className="text-4xl md:text-5xl font-bold text-white drop-shadow">Nettoyage & Réparation de fuites</h1>
            <p className="mt-3 text-white/95 max-w-2xl mx-auto drop-shadow">Démoussage, traitements adaptés et interventions rapides en cas de fuite.</p>
          </div>
        </div>
      </section>
      ...
      <section className="mt-10 grid md:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          <img src="/real5.jpg" alt="Nettoyage de toiture" className="rounded-2xl shadow object-cover w-full h-64" loading="lazy" />
        </div>
      </section>

      <section className="mt-12 grid md:grid-cols-3 gap-6">
        {["Prévention", "Esthétique", "Rapidité"].map((t, i) => (
          <div key={i} className="rounded-2xl bg-white border border-slate-200 shadow p-6">
            ...
            <p className="text-gray-600">{t === 'Prévention' ? 'Évite les infiltrations et dégradations structurelles.' : t === 'Esthétique' ? 'Redonne un aspect propre et net au toit.' : 'Intervention rapide dans votre zone.'}</p>
          </div>
        ))}
      </section>
